...
const Pages = (props) => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (cur === showItem){
                temp.push({num : showItem, isCurrent : true});
            } else {
                temp.push({num : showItem, isCurrent : false});
            }
            setItems(temp);
        }
    }, [cur])
            ...
            <span className="pagnation-columns">
                {
                    items.map(item => {
                        return (
                            <a key={item.num} onClick={() => handleClick(item.num)} className={item.isCurrent ? 'num current' : 'num'}>{item.num}</a>
                        )
                    })
                }
            </span>
